...
const Beams: FC<BeamsProps> = ({
  beamWidth = 2,
  beamHeight = 15,
  beamNumber = 6, // Reduced from 12 to 6
  lightColor = "#ffffff",
  speed = 2,
  noiseIntensity = 1.75,
  scale = 0.2,
  rotation = 0,
  heightSegments = 20, // Reduced from 100 to 20
}) => {
...
>(({ material, width, count, height, segments }, ref) => {
  const mesh = useRef<Mesh<BufferGeometry, ShaderMaterial>>(null!);
  useImperativeHandle(ref, () => mesh.current);
  
  const geometry = useMemo(
    () => createStackedPlanesBufferGeometry(count, width, height, 0, segments),
    [count, width, height, segments]
  );
  ...
  // Optimized animation with accumulated time
  const accumulatedTime = useRef(0);
  useFrame((_, delta) => {
    accumulatedTime.current += delta * 0.1;
    mesh.current.material.uniforms.time.value = accumulatedTime.current;
  });
